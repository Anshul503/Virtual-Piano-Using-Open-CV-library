...
def draw_piano(frame):
    cv2.rectangle(frame, (x1, y1), (x1 + w, y1 + h), (255, 255, 255), -1)# -1 means coloured completely filled with white
...
def key_press(frame,x,y,w1,h1):
    if x>x1 and y>y1 and x+w1<(x1 + w) and y+h1<(y1+h):
        cv2.rectangle(frame, (x1, y1), (x1 + w, y1 + h), (211,211,211), -1) #When it comes in first rectangle,change the color of first rectangle to grey
